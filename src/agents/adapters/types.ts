...
export interface AgentAdapter {
  ...
  runTask(run: AgentRun, prompt: string, signal?: AbortSignal, onChunk?: (chunk: string) => void): Promise<{ exitCode: number | null; output: string }>;
  /** Resume a previous session with a follow-up prompt. */
  resumeTask?(run: AgentRun, followUp: string, signal?: AbortSignal, onChunk?: (chunk: string) => void): Promise<{ exitCode: number | null; output: string }>;
