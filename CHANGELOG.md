...
### Improvements
- **Reasoning flows through the chat instead of a fixed bottom box**: The model's thinking/reasoning text now streams in-flow inside the live assistant bubble — above the tool-call cards and the growing answer — so it scrolls up naturally as the reply is generated, instead of accumulating in a 200px box pinned below the streamed answer. When the turn completes, the reasoning collapses into a “💭 Thinking” disclosure above the final message (expandable, transient — never persisted into session history).
