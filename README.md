...
- **Maxed-out turns conclude instead of erroring**: When a turn exhausts its iteration budget mid-work, the AI writes a final chat message saying it hit the limit and summarizing what it did and what's left (one wrap-up round-trip; a deterministic conclusion takes over if that fails) — no more bare "agentic loop exceeded N iterations" errors, and a limit-stopped turn never fires plan-ready / task-proposal outcomes
- **Iteration budget counts model round-trips, not tool calls**: `adoCode.act.toolBudget` is consumed per model round-trip — one thinking iteration that runs 5 parallel tool calls costs ONE iteration, and the logs and the limit conclusion report the same round-trip count, never the tool-call count
- **Structure-first, read-lazy codebase study**: The AI now orients on directory structure and docs (the cached Repository Understanding, `list_workspace`, README/AGENTS.md/docs) before implementation files, then reads lazily with targeted `read_file` ranges or `search_files` instead of bulk whole-file reads — saving context on long turns; the same guidance reaches delegated agents and the cached repository summaries
...
- **Agentic turns leave an ordered record that stays visible**: Reasoning streams in-flow as separate “Thinking” blocks — one per loop iteration, each sitting directly above the tool batch it introduced, with running → completed tool cards following their block in chronological order (thinking stays between the tool calls instead of gluing into one run-on block). When the loop ends the same record stays in the thread: every thinking block and tool card remains visible and open (collapsible), followed by the final answer. Turns also always conclude — a turn with no closing text (or an empty provider final) gets a deterministic conclusion instead of vanishing
- **Two-sided chat layout**: AI answers render on the left; your messages appear on the right as a compact button-colored bubble that hugs its content — standard chat grammar with author labels (ADO Code / You) in each message's header row, so who said what reads at a glance
- **No more (AI)/(You) label circles**: The little circles that carried the speaker label next to each message are gone — every message already names its author above the content, so the circles only wasted a column of width on each row
...
- **Agent detection matches what your terminal can run**: Windows detection now walks a fallback chain (`claude.cmd` → `claude.exe` → bare name) and records the exact executable found, so native `claude.exe` installs are detected (not just npm shims) — and delegation spawns that same resolved binary instead of a hard-coded bare name (which failed with ENOENT on native Windows npm installs; `.cmd` shims run through `cmd.exe` with proper quoting). Results also re-probe after 15s instead of being cached for the whole session, so an agent installed while VS Code is open shows up without a reload
